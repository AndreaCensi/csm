...
<subsubsection number=no>Acknoweledgements</subsubsection>
...
<subsubsection number=no>Feature #1: Automatic (de)allocation of matrixes</subsubsection number=no>
...
<subsubsection number=no>Feature #2: Caching of matrixes</subsubsection number=no>
...
<subsubsection number=no>Start with a <tt>push()</tt>, end with a <tt>pop()</tt></subsubsection>
...
This is the best that we can do withouth operator overloading. 

Notice that for evaluating this expression `EGSL` needs a lot of 
intermediate matrixes, and it's managing all memory allocations for you.
...
<subsubsection number=no>Allocation of matrixes</subsubsection>

* `zeros(rows, cols)` (`egsl_zeros`): Allocates a matrix with all zeros.
* `ones(rows, cols)` (`egsl_ones`): Allocates a matrix with all zeros.

<subsubsection number=no>Operation with matrixes</subsubsection>

All this functions return a new `val`:

* `sum(A, B)` (`egsl_sum`):  Sums two matrixes.
